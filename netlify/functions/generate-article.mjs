...
${formatInningLine(proofData.homeTeam, homeInnings, homeScore)}${proofData.homeHits ? ' H:'+proofData.homeHits : ''}${proofData.homeErrors ? ' E:'+proofData.homeErrors : ''}
...
// ── Baseball Boxscore Image Handler ──────────────────────────────────────────
async function handleBaseballBoxscore(body, headers) {
  const { fileBase64, fileMimeType, gameData, notes, photographerName, schoolData } = body;

  if (!fileBase64 || !gameData) {
    return { statusCode: 400, headers, body: JSON.stringify({ error: 'fileBase64 and gameData required' }) };
  }
